package com.ekit.weathercastlive.service;
...
import com.ekit.weathercastlive.ConnectionDetector;
import com.ekit.weathercastlive.MainActivity;
import com.ekit.weathercastlive.utils.Constants;
...
import com.ekit.weathercastlive.utils.Utils;
    ...
    public void parseWeather(String data) {
        try {
            ...
            if (weatherObject.has("description")) {
                MainActivity.mWeather.currentWeather.setDescription(weatherObject.getString("description"));
            }
            if (weatherObject.has("icon")) {
                MainActivity.mWeather.currentWeather.setIdIcon(weatherObject.getString("icon"));
            }

            JSONObject mainObj = jsonObject.getJSONObject("main");
            if (mainObj.has("temp")) {
                MainActivity.mWeather.temperature.setTemp(Float.parseFloat(mainObj.getString("temp")));
            }
            if (mainObj.has("pressure")) {
                MainActivity.mWeather.currentCondition.setPressure(
                        Float.parseFloat(mainObj.getString("pressure")));
            }
            if (mainObj.has("humidity")) {
                MainActivity.mWeather.currentCondition.setHumidity(mainObj.getInt("humidity"));
            }

            JSONObject windObj = jsonObject.getJSONObject("wind");
            if (windObj.has("speed")) {
                MainActivity.mWeather.wind.setSpeed(Float.parseFloat(windObj.getString("speed")));
            }
            if (windObj.has("deg")) {
                MainActivity.mWeather.wind.setDirection(Float.parseFloat(windObj.getString("deg")));
            }

            JSONObject cloudsObj = jsonObject.getJSONObject("clouds");
            if (cloudsObj.has("all")) {
                MainActivity.mWeather.cloud.setClouds(cloudsObj.getInt("all"));
            }
            ...
            if (sysObj.has("country")) {
                MainActivity.mCitySearch.setCountryCode(sysObj.getString("country"));
            }
            MainActivity.mWeather.sys.setSunrise(sysObj.getLong("sunrise"));
            MainActivity.mWeather.sys.setSunset(sysObj.getLong("sunset"));
